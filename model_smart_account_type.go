/*
MSX SDK

MSX SDK client.

API version: 1.0.10
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package msxsdk

import (
	"encoding/json"
	"fmt"
)

// SmartAccountType Smart Account Types
type SmartAccountType string

// List of SmartAccountType
const (
	SMARTACCOUNTTYPE_CUSTOMER SmartAccountType = "CUSTOMER"
	SMARTACCOUNTTYPE_HOLDING SmartAccountType = "HOLDING"
)

// All allowed values of SmartAccountType enum
var AllowedSmartAccountTypeEnumValues = []SmartAccountType{
	"CUSTOMER",
	"HOLDING",
}

func (v *SmartAccountType) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := SmartAccountType(value)
	for _, existing := range AllowedSmartAccountTypeEnumValues {
		if existing == enumTypeValue {
			*v = enumTypeValue
			return nil
		}
	}

	return fmt.Errorf("%+v is not a valid SmartAccountType", value)
}

// NewSmartAccountTypeFromValue returns a pointer to a valid SmartAccountType
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewSmartAccountTypeFromValue(v string) (*SmartAccountType, error) {
	ev := SmartAccountType(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for SmartAccountType: valid values are %v", v, AllowedSmartAccountTypeEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v SmartAccountType) IsValid() bool {
	for _, existing := range AllowedSmartAccountTypeEnumValues {
		if existing == v {
			return true
		}
	}
	return false
}

// Ptr returns reference to SmartAccountType value
func (v SmartAccountType) Ptr() *SmartAccountType {
	return &v
}

type NullableSmartAccountType struct {
	value *SmartAccountType
	isSet bool
}

func (v NullableSmartAccountType) Get() *SmartAccountType {
	return v.value
}

func (v *NullableSmartAccountType) Set(val *SmartAccountType) {
	v.value = val
	v.isSet = true
}

func (v NullableSmartAccountType) IsSet() bool {
	return v.isSet
}

func (v *NullableSmartAccountType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSmartAccountType(val *SmartAccountType) *NullableSmartAccountType {
	return &NullableSmartAccountType{value: val, isSet: true}
}

func (v NullableSmartAccountType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSmartAccountType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

